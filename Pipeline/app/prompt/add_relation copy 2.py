...
You must check the actual distance and rotations between objects before assigning a relation. 
If the child and parent are not **very close** (i.e. the bboxes' edges are more than 1.0 meter apart) or the rotation does not align with the relation, **you MUST check the necessity of assigning the relation** between them.
...
You must strictly enforce spatial proximity when assigning relations. If two objects's edges are far away from each other, DO NOT assign any relation between them, regardless of their category or orientation.
