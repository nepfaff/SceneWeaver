...
###################################
                ...
                """)
            tabletype_ = tabletype.replace(" ","_")
            obj_caption_prompt_payload = gpt.payload_get_object_caption(img_path=input_path,tabletype=tabletype_)
            iter = 0
            while True and iter<5:
                gpt_text_response = gpt(payload=obj_caption_prompt_payload, verbose=self.verbose)
                if tabletype_ in gpt_text_response:
                    break
                else:
                    iter += 1
                    print("The supporter is not detected by GPT")
            if iter==5:
                with open("args.json","w") as f:
                    j = json.load(f)
                    j["success"] =  False
                    j["error"] =  "The supporter is not detected by GPT"
                raise 
            # gpt_text_response = '[desk, pan, steak, spatula, seasoning, paper towel]' #'[desk,juicer,orange,knife,glass]'
            # gpt_text_response = '[tv stand, television, vase of flower, vase of flower]'
            # gpt_text_response = '[desk, monitor, mouse, keyboard, laptop, cup, bottle, tissue box, hand sanitizer, disinfectant spray, snack bag, blanket, drawing, plush toy, figurine, document]'
            # gpt_text_response = '[desk, book, paper, glove, sunglass, phone, cable]'
            # gpt_text_response = '[desk, monitor, mouse, keyboard]'
            # gpt_text_response = '[desk, monitor, keyboard, mouse, mouse pad, tablet, plant, bottle, desk mat]' #desk6 :
            # gpt_text_response = '[desk, monitor, keyboard, book, pot plant]' # 111 
            # gpt_text_response = '[kitchen island, bread, bowl, scale]' kitchen4
            # gpt_text_response = '[desk, monitor, headphone, coffee cup, keyboard, mouse, notebook, pen]' #desk3
...
##################################################################
        ...
        #         # gpt_text_response = 'The mask in the second image highlights the wall behind'
        #         if gpt_text_response is None:
        #             # Failed, terminate early
        #             return False, None
...
####################################################################
        ...
        if tabletype != tabletype_:
            phrases = [p.replace(" _ "," ") for p in phrases]
            detected_objs[tabletype] = detected_objs[tabletype_]
            del detected_objs[tabletype_]
...
##############################################################################################
        ...
        if tabletype not in category_counts:
            with open("args.json","r") as f:
                j = json.load(f)
                j["success"] =  False
                j["error"] =  "Supporter not detected."
            with open("args.json","w") as f:
                json.dump(j,f,indent=4)
            raise AssertionError
        
        if category_counts[tabletype]>1:
            with open("args.json","r") as f:
                j = json.load(f)
                j["success"] =  False
                j["error"] =  "More than 1 supporter."
            with open("args.json","w") as f:
                json.dump(j,f,indent=4)
            raise AssertionError
...
########################################################################
        ...
        #     phrases_recaptioned[i] = clean_caption
            ...
            annotated_frame = annotate(image_source=image_source, boxes=box[None, :], phrases=[phrase])
            cv2.imwrite(annotated_bbox_img_path, annotated_frame)
        ...
        for i, (mask, name, clean_caption) in enumerate(zip(masks, names, phrases_recaptioned)):
            obj_mount_info = {"floor": False, "wall": None}         # default info
            if clean_caption == tabletype:
