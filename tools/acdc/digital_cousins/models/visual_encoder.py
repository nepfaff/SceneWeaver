...
class VisualEncoder(torch.nn.Module):
    ...
    def get_features(self, x, input_category):
        """
        ...
        with torch.inference_mode():
            ...
            if self.batch_size is not None:
                ...
                for i in range(n_batches):
                    ...
                    feature_batch = self.forward(batch, input_category).detach().cpu().numpy()
                    feature_batches.append(feature_batch)
                    torch.cuda.empty_cache()
                features = np.concatenate(feature_batches, axis=0)
            else:
                features = self.forward(preprocessed_images,input_category).detach().cpu().numpy()
                torch.cuda.empty_cache()
